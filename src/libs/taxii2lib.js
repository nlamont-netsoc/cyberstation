    ...
    //------------------------------------------------------------------
    //------------------------------------------------------------------
    // NOTE THE PROXY    const proxyurl = "https://cors-anywhere.herokuapp.com/";
    // IN                fetchTimeout(url, options, timeout, error)
    //------------------------------------------------------------------
    //------------------------------------------------------------------
    //------------------------------------------------------------------
...
export class TaxiiConnect {
    ...
    //------------------------------------------------------------------
        ...
        return this.timeoutPromise(fetch(proxyurl+url, options), timeout, error); // <----- proxyurl
    }
    ...
    async asyncFetch(path, config, filter) {
        ...
        return await (await (
            this.fetchTimeout(fullPath, config, this.timeout, 'connection timeout')
                .then(res => res.json())
                .catch(err => { throw Error("fetch error: " + err); }) ));
    }
